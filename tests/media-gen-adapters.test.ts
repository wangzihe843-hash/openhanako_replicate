...
let mediaDataDir: string;
beforeEach(() => {
  mediaDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "hana-media-adapters-"));
});
afterEach(() => {
  const resolved = path.resolve(mediaDataDir);
  if (path.dirname(resolved) !== path.resolve(os.tmpdir())
    || !path.basename(resolved).startsWith("hana-media-adapters-")) {
    throw new Error("Refusing to remove a directory outside the media test fixture");
  }
