...
- **reasonText** 会作为 TA 状态里的「变化原因」展示给用户——用 TA 第一人称、贴角色口吻写一句心里话「为什么这会儿对 user 的心绪变成了这样」，像一片真实的私密残片，带点温度与心意；**不要**写成「源自哪段聊天 / 哪个事件」的数据溯源，也不要「互动稳定」这类系统说明（要追溯出处就填 `sourceEventIds`）。例：「你今天主动留下来等我吃完饭——你从前不会这样，我一时没敢信，又偷偷高兴了好久」。可以与顶层 `reason` 并存（UI 优先展示 reasonText）。
...
relationship_state:
  ...
  reasonText: "<TA 第一人称、带心意的一句心里话：为什么这会儿对你的心绪变成了这样；不要写成溯源式说明, 可选>"
```
