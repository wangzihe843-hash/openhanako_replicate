...
export function buildMomentDraftPrompt(args: {
  ...
  const parts: string[] = [
    ...
    '',
    '同时生成围观互动（likes / comments，可选）：',
    '- ref 必须严格取自下方「可选互动者池」中已列出的 ref；不要凭空捏造 ref 或 displayName。',
    '- 不要把当前角色自己、user / 莉莉丝 / 任何用户身份放进 likes 或 comments（用户的点赞评论由 UI 触发）。',
    '- likes 至多 4 条；comments 至多 3 条，每条 body 控制在 30 字以内、口语化、符合该互动者口吻。',
    '- 池子为空（「（无）」）时直接省略对应字段，不要输出空数组占位。',
    '',
    '输出 JSON schema（仅此结构，字段名必须一致；除 content 外其余字段可选）：',
    JSON.stringify(
      {
        content: 'string',
