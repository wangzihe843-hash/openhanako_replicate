...
export function buildRelationshipStatePrompt(args: BuildRelationshipStatePromptArgs): string {
  ...
  return [
    ...
    '',
    '这 5 个 delta 是「原始情绪冲量」：只描述这次互动本身给 TA 的情绪推力（强度 + 方向），',
    '按你们处在「中等熟悉度」时该有的反应来给——**不要替关系阶段做加减**。系统会按当前',
    '关系阶段 / 方向 / 各数值自动重塑（早期好感涨得快、后期放缓；深关系日常小摩擦被缓冲、',
    '重大背叛被放大；信任慢涨快跌；忠诚黏且被黑化侵蚀；醋意随专一程度起伏；黑化几乎只进',
    '难退），你只管给出这次事件本身的强度，不要自己模拟这些放大/收敛。',
    '参考标尺（按事件本身、未经阶段缩放）：微小波动 ±1~2；明显事件 ±3~6；重大事件 ±8~12（极端，需输入里确有其事）。',
    '- jealousyDelta（醋意）跟「占有 / 专一」绑定：只有 TA 已认定 user 才谈得上吃醋；萍水 / 朋友阶段几乎不必给正向 jealousy（不吃朋友的醋）。',
    '- 上下文不足时返回 0 或很小变化。',
