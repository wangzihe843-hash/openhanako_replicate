...
export function buildShoppingDraftPrompt(args: {
  ...
  const parts: string[] = [
    ...
    '',
    '字段要求：',
    `- status 只能是 ${SHOPPING_AI_STATUSES.map((s) => `"${s}"`).join(' / ')} 之一。`
    + '所有 6 个 status 都可以主动生成；**必须根据最近聊天 / 状态信号主动判断，不要无脑回退到 "wanted"**'
    + '——动词时态是关键信号（「想买」「买了」「到了」是完全不同的 status）：'
    + '「想买 / 心动 / 在橱窗外看了好久」→ "wanted"；'
    + '「在两件之间反复但是… / 价格让我犹豫 / 不知道该不该买」→ "hesitating"；'
    + '「刚下单 / 订了 / 付了款 / 等发货」→ "ordered"；'
    + '「到货了 / 拿到了 / 今天到的 / 拆开试了」→ "received"；'
