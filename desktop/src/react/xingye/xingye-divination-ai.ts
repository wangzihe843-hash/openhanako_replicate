 ...
 *
 * 除了"避免重复"，本 anchor 还开一个**轻度回扣**的口子：允许新占卜在正文里
 * 点名最近 1–2 次抽到的牌/卦象/签象，写一句「上次……是因为……，这次……」作对照
 * （点名旧符号是为了自然岔开这次的解读，不等于这次还抽它——这次仍要换牌/换卦）。
 * 回扣至多一句，标题 / 牌面卦象 / 正文 / 行动签仍须全部另起，严禁照搬旧条目的任何
 * 整句正文或整组符号。这段边界文案随 anchor 一起下发，只在「有历史」时出现——
 * 第一次占卜没有可回扣的旧条目，prompt 端直接渲染「第一次占卜」占位。
 *
 * @param agentId 角色 id
...
export async function buildDivinationContinuityAnchorBlock(
  ...
  try {
    ...
    // 轻度回扣：在"避免重复"之外，授予新占卜引用旧占卜作对照的口子。samples 走到这里
    // 必非空（上面有 `if (!samples.length) return ''` 守卫），故 callback 提示无条件下发。
    // 边界写死在提示里：点名旧符号 OK，照搬整句/整组符号绝对不行——和上面"硬避免"自洽
    //（点名上次的牌正是为了说"这次别再落在它上面"）。
    lines.push(
      '- 可轻度回扣（非必须，自然时才提）：允许在正文里点名上面最近 1–2 次抽到的某一张牌/某一卦象/某一签象，写一句「上次……是因为……，这次……」作对照，借此自然地岔开这次的解读；点名旧符号只是提一句旧结果，这次仍要换牌/换卦，不是再抽它。',
      '  边界：回扣至多一句；本次的标题、牌面/卦象组合、正文、行动签都必须另起——严禁照搬旧条目的整句正文或整组符号，任何一段与旧条目字面雷同都不允许。',
    );
    if (filterMethod) {
      lines.push(
