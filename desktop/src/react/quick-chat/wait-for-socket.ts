...
export function waitForSocketOpen(ws: WebSocket, signal: AbortSignal, timeoutMs = 30_000): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    if (ws.readyState === WebSocket.OPEN) { open(); return; }
    if (ws.readyState !== WebSocket.CONNECTING) { fail(); return; }
    ws.addEventListener('open', open);
    ws.addEventListener('error', fail);
    ws.addEventListener('close', fail);
    signal.addEventListener('abort', cancel, { once: true });
    timer = setTimeout(() => { cleanup(); reject(new Error('WebSocket connection timed out')); }, timeoutMs);
  });
}
