...
function installStoreMethods() {
  ...
  s.clearQuotedSelection = vi.fn(() => {
    mockState.quoteCandidate = null;
    mockState.quotedSelections = [];
    mockState.quotedSelection = null;
  });
  ...
  s.setSessionWorkMode = vi.fn((enabled: boolean) => { mockState.sessionWorkMode = enabled === true; });
...
function mockPermissionDefault(mode = 'ask') {
  ...
  it('clears unread output marker when switching into a normal session', async () => {
    Object.assign(mockState, {
      ...
      quotedSelections: [{ text: 'quote from current session' }],
      quotedSelection: { text: 'quote from current session' },
      stagedChatQuote: { text: 'xingye handoff' },
    });
    mockFetch.mockImplementation(async (url: string) => {
    ...
    expect(mockState.quotedSelections).toEqual([]);
    expect(mockState.quotedSelection).toBeNull();
    expect(mockState.stagedChatQuote).toEqual({ text: 'xingye handoff' });
  });
  ...
  describe('createNewSession cwd draft', () => {
    it('resets work mode while the new session is still pending', async () => {
      (mockState as Record<string, unknown>).sessionWorkMode = true;
