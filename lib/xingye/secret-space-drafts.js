 ...
 *  - 限制 category 为三个最纯文本、不与其它模块语义重叠的子集：
 *      state / dream / saved_item
 *    （`draft_reply` 与 mail.draft 接近；`unsent_moment` 与 moments.draft 接近；
 *     `memory_fragment` 走 MemoryCandidatePanel——目前是用户手动点 AI 生成候选
 *     + 人工 confirm/reject 的二阶段流程，**不是 agent 主动产出**。这条以后可能
 *     也接进 xingye_propose_draft 让 agent 在巡检里主动提议 memory candidate，
 *     当前先用 MemoryCandidatePanel 已有路径。）
 ...
 * @param {{
 *   category: 'state' | 'dream' | 'saved_item',
 *   title?: string,
 *   body: string,
 *   tags?: string[],
 *   reason?: string,
...
export async function appendSecretSpaceDraftServer({ agentDir, agentId, input }) {
  ...
  return { id, category, title, body, tags, createdAt, reason, source, sourceEventIds };
}
