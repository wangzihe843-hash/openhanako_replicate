...
/**
 * 允许的收支方向。dispatch 边界（xingye-propose-draft-tool）用它做主校验——
 * 非法 / 缺省的 direction 在那里直接 ok:false 退回让 agent 重发，避免被
 * 下方 normalizeDirection 静默兜成 'expense' 翻转账本符号。这里仍保留
 * normalizeDirection 作为防御性后备。
 */
export const ACCOUNTING_DRAFT_ALLOWED_DIRECTIONS = Object.freeze(["income", "expense"]);
const ACCOUNTING_DIRECTIONS = new Set(ACCOUNTING_DRAFT_ALLOWED_DIRECTIONS);
const TITLE_MAX = 80;
