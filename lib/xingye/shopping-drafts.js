...
/**
 * 允许的购物状态。dispatch 边界（xingye-propose-draft-tool）用它做主校验——
 * 非法的 status 在那里直接 ok:false 退回让 agent 重发，避免被下方
 * normalizeStatus 静默兜成 'wanted'。这里仍保留 normalizeStatus 作为
 * 防御性后备（也覆盖「缺省 → wanted」这一合法默认）。
 */
export const SHOPPING_DRAFT_ALLOWED_STATUSES = Object.freeze(["wanted", "ordered", "received", "hesitating", "returned", "favorite"]);
const SHOPPING_STATUSES = new Set(SHOPPING_DRAFT_ALLOWED_STATUSES);
