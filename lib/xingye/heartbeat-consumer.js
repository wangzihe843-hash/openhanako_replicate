...
} from "../conversations/agent-phone-projection.ts";
...
/** Historical interaction threshold, retained for compatibility and optional review.
 * `mustPropose` is a legacy field name: it never mandates speaking or a draft.
 */
export const XINGYE_AUTO_DRAFT_STALENESS_THRESHOLD = 50;
const XINGYE_AUTO_DRAFT_COUNTER_ID = "xingye.auto-draft-counter";
...
function readPeerSocialFallbackSettings(agentDir, peerId) {
  try {
    const projection = readAgentPhoneProjection(
      getAgentPhoneProjectionPath(agentDir, `dm:${peerId}`),
    );
    const rawMode = typeof projection?.meta?.socialFallbackMode === "string"
      ? projection.meta.socialFallbackMode.trim().toLowerCase()
      : "";
    const socialFallbackMode = ["auto", "enabled", "disabled"].includes(rawMode)
      ? rawMode
      : "auto";
    const rawInterval = Number(projection?.meta?.socialFallbackTurnInterval);
    return {
      socialFallbackMode,
      socialFallbackTurnInterval: Number.isFinite(rawInterval) && rawInterval > 0
        ? Math.floor(rawInterval)
        : null,
    };
  } catch {
    return { socialFallbackMode: "auto", socialFallbackTurnInterval: null };
  }
}
...
const TYPE_LABEL_ZH = {
  ...
  "trips.entry_appended": "行程新增",
  "trips.entry_deleted": "行程删除",
  "trips.draft_proposed": "行程草稿提议",
  "trips.draft_discarded": "行程草稿丢弃",
  "trips.draft_confirmed": "行程草稿确认",
...
async function readEventLog(agentDir, agentId) {
  ...
  return {
    ...
    log: {
      ...
      ...(isRecord(raw) && Object.hasOwn(raw, "autoDraftState") ? { autoDraftState: raw.autoDraftState } : {}),
    },
  };
}
...
/**
 * Count each observation once in the same atomic document as its receipt.
 * Receipts use consumedBy and disappear with ordinary event retention; no
 * unbounded ID index or timestamp watermark can discard late observations.
 * Legacy logs seed only the retained-history lower bound.
 */
function advanceAutoDraftState(events, previous, agentId) {
  if (previous != null && (
    !isRecord(previous) || previous.version !== 1 || previous.agentId !== agentId
    || !Number.isSafeInteger(previous.userTurnCount) || previous.userTurnCount < 0
    || !Number.isSafeInteger(previous.lastAutoDraftTurn) || previous.lastAutoDraftTurn < 0
    || previous.lastAutoDraftTurn > previous.userTurnCount
    || (previous.lastAutoDraftAt !== null && !normalizeDateString(previous.lastAutoDraftAt))
    || (previous.lastPrunedChatAt != null && !normalizeDateString(previous.lastPrunedChatAt))
    || (previous.baselineUncertain != null && typeof previous.baselineUncertain !== "boolean")
  )) throw new Error("invalid Xingye auto-draft counter state");

  const chats = new Map();
  const countedIds = new Set();
  let latestDraftAt = previous?.lastAutoDraftAt ?? null;
  for (const event of events) {
    if (event.type === "recent_chat.observed") {
      if (!chats.has(event.id)) chats.set(event.id, event);
      if (event.consumedBy?.[XINGYE_AUTO_DRAFT_COUNTER_ID]) countedIds.add(event.id);
    } else if (event.type.endsWith(".draft_proposed")) {
      if (!latestDraftAt || Date.parse(event.createdAt) > Date.parse(latestDraftAt)) {
        latestDraftAt = event.createdAt;
      }
    }
  }

  const newChats = [...chats.values()].filter((event) => !previous || !countedIds.has(event.id));
  const userTurnCount = (previous?.userTurnCount ?? 0) + newChats.length;
  if (!Number.isSafeInteger(userTurnCount)) throw new Error("Xingye user-turn counter overflow");
  const cutoff = latestDraftAt ? Date.parse(latestDraftAt) : -Infinity;
  // A newly observed draft resets the baseline according to event time, so
  // same-millisecond chats are excluded in either arrival order (legacy rule).
  const changedDraft = !previous || latestDraftAt !== previous.lastAutoDraftAt;
  const predatesPrunedChats = previous?.lastPrunedChatAt
    && cutoff < Date.parse(previous.lastPrunedChatAt);
  // A late draft cannot reconstruct a baseline from chats already pruned.
  // Preserve accumulated progress conservatively until a fresh draft supplies
  // an exact baseline; this watermark NEVER rejects a late chat observation.
  const baselineUncertain = changedDraft
    ? !!predatesPrunedChats
    : previous?.baselineUncertain === true;
  const elapsed = changedDraft && !baselineUncertain
    ? [...chats.values()].filter((event) => Date.parse(event.createdAt) > cutoff).length
    : previous.userTurnCount - previous.lastAutoDraftTurn
      + newChats.filter((event) => Date.parse(event.createdAt) > cutoff).length;
  return {
    version: 1,
    agentId,
    userTurnCount,
    lastAutoDraftAt: latestDraftAt,
    lastAutoDraftTurn: Math.max(0, userTurnCount - elapsed),
    lastPrunedChatAt: previous?.lastPrunedChatAt ?? null,
    baselineUncertain,
  };
}
...
async function markConsumed(logPath, log, consumedEventIds, consumedAt, autoDraftState) {
  const ids = new Set(consumedEventIds);
  const updated = log.events.map((event) => {
    const chatObservation = event.type === "recent_chat.observed";
    if (!ids.has(event.id) && !chatObservation) return event;
    return {
      ...event,
      consumedBy: {
        ...(event.consumedBy || {}),
        ...(ids.has(event.id) ? { [XINGYE_HEARTBEAT_CONSUMER_ID]: consumedAt } : {}),
        ...(chatObservation ? { [XINGYE_AUTO_DRAFT_COUNTER_ID]: event.consumedBy?.[XINGYE_AUTO_DRAFT_COUNTER_ID] || consumedAt } : {}),
      },
    };
  });
  ...
  const retainedIds = new Set(prunedEvents.map((event) => event.id));
  let lastPrunedChatAt = autoDraftState.lastPrunedChatAt;
  for (const event of updated) {
    if (event.type !== "recent_chat.observed" || retainedIds.has(event.id)) continue;
    if (!lastPrunedChatAt || Date.parse(event.createdAt) > Date.parse(lastPrunedChatAt)) {
      lastPrunedChatAt = event.createdAt;
    }
  }
  await atomicWriteJson(logPath, {
    ...log,
    autoDraftState: { ...autoDraftState, lastPrunedChatAt },
    events: prunedEvents,
    dedupeKeys: prunedDedupeKeys,
  });
}

/** 返回去掉 candidatePeers[].relationshipLore 的 socialStaleness 副本（持久化用，prompt-only 字段不落盘）。 */
function stripCandidateLore(socialStaleness) {
  if (!socialStaleness || !Array.isArray(socialStaleness.candidatePeers)) return socialStaleness;
  return {
    ...socialStaleness,
    candidatePeers: socialStaleness.candidatePeers.map((peer) => {
      const persistedPeer = { ...peer };
      delete persistedPeer.relationshipLore;
 ...
 *   当前可联系的其他 agent（**应已排除自己**）。用来算 social staleness。
 *   不传 → socialStaleness.shouldSocialize 恒为 false（没人可聊），向后兼容旧调用方。
 * @param {{globalThreshold:number, perPeerThreshold:number}} [opts.socialThresholds]
 *   社交 staleness 阈值（来自 agent config，见 resolveSocialThresholds）。不传则
 *   computeSocialStaleness 用内置默认常量。
 * @param {() => Date} [opts.now]
 */
export async function runXingyeHeartbeatConsumer({ agentId, agentDir, peers, socialThresholds, now = () => new Date() } = {}) {
  ...
  return withXingyeAgentEventLock(aid, async () => {
    ...
    /**
     * Staleness 基于持久化总计数及本轮尚未计数的 recent_chat 事件，保留草稿基准；
     * 七天日志清理不会重置累计进度。旧日志只迁移仍保留的历史下界，不虚构已删历史。
     * 计数与 consumedBy 回执原子写入同一文件，重复消费不会重复累加。
     *
     * 即使本轮没有新事件也照算并下发：staleness 跟 "本轮是否有事可看" 无关，agent
     * 计数只作参考，不构成主动表达的理由。
     */
    const autoDraftState = advanceAutoDraftState(log.events, log.autoDraftState, aid);
    const elapsedDraftTurns = autoDraftState.userTurnCount - autoDraftState.lastAutoDraftTurn;
    const autoDraftStaleness = {
      lastAutoDraftAt: autoDraftState.lastAutoDraftAt,
      chatTurnsSinceLastDraft: elapsedDraftTurns,
      mustPropose: elapsedDraftTurns >= XINGYE_AUTO_DRAFT_STALENESS_THRESHOLD,
    };

    /**
     * Social staleness 同样通过持久化计数累计 recent_chat.observed，
     * 不依赖本轮是否有新事件。peerState 记录「上次主动
     * dm 谁」（dm-tool 写入），peers 是当前可联系列表（scheduler 注入，已排除自己）。
     */
    // “保底社交”只面向已经有明确 peer 关系设定的人。没有关系 lore 的 agent 仍可被
    // 用户点名或由角色自然主动联系，但不会仅因 user 又聊了 N 条就被系统硬塞进候选。
    // 关系在 compute 前现读，才能让 unrelated peers 真正不参与 global/per-peer 阈值。
    let relationshipEntries = [];
    try {
      relationshipEntries = readXingyeRuntimeLoreEntriesSync({
        hanakoHome: path.dirname(path.dirname(dir)),
        agentId: aid,
        agentDir: dir,
      });
    } catch {
    ...
    const perPeerLoreBudget = Math.max(240, Math.floor(4_800 / Math.max(1, safePeers.length)));
    const relationshipAwarePeers = safePeers.map((peer) => {
      const socialFallback = readPeerSocialFallbackSettings(dir, peer?.id);
      const relationshipLore = buildXingyePeerRelationshipLore({
        agentId: aid,
        agentDir: dir,
        hanakoHome: path.dirname(path.dirname(dir)),
        peerName: peer?.name,
        ...
        maxChars: perPeerLoreBudget,
      });
      return {
        ...peer,
        ...socialFallback,
        relationshipKnown: !!relationshipLore,
        ...(relationshipLore ? { relationshipLore } : {}),
      };
    });
    const socialStaleness = computeSocialStaleness({
      events: log.events,
      // Persist a monotonic user-turn sequence before event retention prunes old
      // chat observations. Per-peer outbound baselines then remain valid forever.
      peerState: syncPeerStateUserTurns({ agentDir: dir, events: log.events }) || readPeerState(dir),
      peers: relationshipAwarePeers,
      globalThreshold: socialThresholds?.globalThreshold,
      perPeerThreshold: socialThresholds?.perPeerThreshold,
    });

    if (events.length === 0) {
      // Migrate already-consumed legacy logs too, and continue ordinary retention.
      if (log.events.length || log.autoDraftState) {
        await markConsumed(logPath, log, [], now().toISOString(), autoDraftState);
      }
      /** 没新事件也要把 staleness 带回去——上层 patrol prompt 可能据此追加 directive。 */
      return { consumed: 0, skipped: true, autoDraftStaleness, socialStaleness };
    }
    ...
    /**
     * relationshipLore 只为本轮 patrol prompt 渲染服务，不该落进 result.json / history.jsonl
     * （那是给渲染端/历史看的，逐 peer 的 lore 全文是纯噪音 + 体积）。持久化前剥掉，
     * 返回给上层的 result 仍带 lore（prompt 用）。
     */
    const persistResult = {
      ...result,
      socialStaleness: stripCandidateLore(socialStaleness),
    };
    await writeHeartbeatResult(dir, persistResult);
    await markConsumed(logPath, log, result.consumedEventIds, createdAt, autoDraftState);
