...
import { readPeerState, computeSocialStaleness } from "../desk/social-awareness.js";
...
export async function runXingyeHeartbeatConsumer({ agentId, agentDir, peers, socialThresholds, now = () => new Date() } = {}) {
  ...
  return withXingyeAgentEventLock(aid, async () => {
    ...
    /**
     * 心跳开场白定向注入：只在社交段会真正渲染时（shouldSocialize 或有逾期 peer），
     * 给被点名的候选 peer 补上「本 agent 对该 peer 的关系 lore」，让主动发起的第一句
     * 也贴合既定关系、不瞎编。读文件有成本，故按需、且只对 top-N 候选读。
     */
    if (socialStaleness && (socialStaleness.shouldSocialize || socialStaleness.overduePeerCount > 0)) {
      for (const cand of socialStaleness.candidatePeers || []) {
        ...
        if (lore) cand.relationshipLore = lore;
      }
    }
