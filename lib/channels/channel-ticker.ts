...
export function createChannelTicker({
  ...
  function pickRandomAgent(channelName) {
    ...
    // 只从「在自己 bookmark 之后还有未读消息」的成员中选；否则会出现：频道
    // 自上次提醒以来没有新活动，但调度器仍每个周期把同一份「最近 20 条」推
    // 给随机成员，让 agent 反复收到一模一样的频道汇总。
    const candidates = getAgentOrder().filter((id) => {
      if (!channelMembers.has(id)) return false;
      const channelsMdPath = path.join(agentsDir, id, "channels.md");
      const bookmark = readBookmarks(channelsMdPath).get(channelName);
      const bk = !bookmark || bookmark === "never" ? null : bookmark;
      return getNewMessages(channelFile, bk, id).length > 0;
    });
    if (candidates.length === 0) return null;
    const idx = Math.min(candidates.length - 1, Math.floor(Math.max(0, Math.min(0.999999, random())) * candidates.length));
  ...
  function triggerReminder(channelName) {
    ...
    _triggerChain = _triggerChain.then(async () => {
      ...
      if (!proactiveAgentId) {
        // 没人有未读：复位 dueAt 推到下一个 interval，否则 dueAt 仍停在过去，
        // _scheduleNext 会立刻 setTimeout(0) 再次触发，形成空响铃紧密循环。
