 ...
 *
 * 沙盒开启时，POSIX/Git runtime 只接受打包进 resources/git 的 bundled runtime。
 * 找不到就 fail fast，不再降级到系统 Git Bash，避免 AppContainer + MSYS2 冲突。
 */
...
function findAndCacheShell(startAfter, options = {}) {
  // 有缓存且不是降级重试 → 直接返回
  if (_cachedShell && !startAfter && !options.preferBundled && !options.bundledOnly) return _cachedShell;
