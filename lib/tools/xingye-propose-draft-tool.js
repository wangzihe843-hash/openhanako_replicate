    ...
    parameters: Type.Object({
      ...
      phone_contact: Type.Optional(Type.Object({
        targetType: StringEnum([...PHONE_CONTACT_DRAFT_ALLOWED_TARGET_TYPES], {
          description: "[phone_contact] 目标联系人的 targetType。允许：agent（真实角色联系人）/ virtual_contact（虚拟联系人）/ user（user 这条特殊联系人，targetId 用 \"__user__\"）。**本模块只生成「对现有联系人的更新候选」**——不能用于新增联系人、不能用于拉黑/删除/恢复（那些走小手机通讯录手动 AI 更新路径）。",
        }),
        targetId: Type.String({
          description: "[phone_contact] 目标联系人的 targetId（virtual_contact 用 virtualContact.id；agent 用 agent.id；user 用 \"__user__\"）。必须是当前通讯录已存在的联系人；不存在的会在 UI 确认阶段被拒。",
        }),
        displayName: Type.Optional(Type.String({
          description: "[phone_contact] 联系人当前的显示名（仅用于 UI 把草稿展示给用户时让 TA 一眼认出是谁），不是要改成的新名字。本工具不接受改 displayName。≤80 字符。",
        })),
        patch: Type.Object({
          ...
          tags: Type.Optional(Type.Array(Type.String(), {
            description: "[phone_contact.patch] 标签数组，**整体替换**（不是增量）。最多 8 个，每个 ≤32 字符。**对 agent / virtual_contact**：仅允许来自固定词表「亲近的人 / 需要观察 / 不可靠 / 同伴 / 危险」——其它会被丢；**对 user**：可以是来自最近聊天的简短自然标签（如「尊重边界 / 不逞强 / 愿意配合」），不强制词表。该约束**调用方应预先按 targetType 选择**——server 端只做整体长度/数量校验。",
          })),
          faction: Type.Optional(StringEnum([...PHONE_CONTACT_DRAFT_ALLOWED_FACTIONS], {
            description: "[phone_contact.patch] 阵营。允许：自己人 / 中立 / 对立 / 未知。其它值会被拒（patch 中视为该字段未给出）。",
          })),
        }, {
          description: "[phone_contact.patch] 更新补丁；**至少必须含一个非空字段**。不允许的字段（displayName / kind / shortBio / avatarDataUrl / linkedAgentId / status / originalName）即使传也会被忽略。**status 故意不开**——把 active 改成 blocked/deleted/restore 不是本候选的职责，走小手机通讯录手动 AI 更新。",
        }),
      }, { description: "module === 'phone_contact' 时的字段。确认后通过 savePhoneContactMeta 合并到现有联系人 meta（markManualFields=true，source='manual'）——用户在 UI 上 review 过了即视为手动认可。" })),
    }),
    execute: async (_toolCallId, params) => {
      ...
      switch (moduleName) {
        ...
        case "phone_contact": {
          ...
          if (!targetId) {
            return {
              content: [{ type: "text", text: "module=phone_contact 缺少 phone_contact.targetId（或为空）；已拒绝写入草稿。" }],
              details: { ok: false, module: "phone_contact", reason: "missing_target_id" },
            };
          }
          const patchRaw = payload.patch && typeof payload.patch === "object" && !Array.isArray(payload.patch) ? payload.patch : null;
          if (!patchRaw) {
            return {
              content: [{ type: "text", text: "module=phone_contact 缺少 phone_contact.patch 对象；已拒绝写入草稿。" }],
          ...
          try {
            ...
            if (!draft) {
              return {
                content: [{
                  type: "text",
                  text: "草稿写入失败：输入校验未通过（agentId / targetType / targetId / patch 字段不合法，或 patch 在归一后没有任何有效字段）。",
                }],
                details: { ok: false, module: "phone_contact", reason: "validation_failed" },
              };
            }
            const fieldsLabel = Object.keys(draft.patch).join(" / ") || "(empty)";
            const who = draft.displayName ? `${draft.displayName}（${draft.targetType}）` : `${draft.targetType}:${draft.targetId}`;
            return {
              content: [{
                type: "text",
                text: `已写入通讯录更新候选 ${draft.id}（${who} · ${fieldsLabel}），待用户在小手机通讯录「待确认草稿」区确认。`,
              }],
              details: {
                ok: true,
                ...
                patchFields: Object.keys(draft.patch),
              },
            };
          } catch (err) {
            const msg = err?.message || String(err);
