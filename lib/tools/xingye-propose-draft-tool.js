 ...
 *
 * 后续候选（当前未接入，但模式高度兼容）：
 *  - `memory_candidate`：MemoryCandidatePanel 当前是「用户手动点 AI 生成候选 +
 *     人工 confirm/reject」的二阶段流程。agent 在巡检里主动 propose memory
 *     candidate 是天然的扩展方向。
 *  - `relationship_state`：RelationshipStatePanel 当前也是「用户手动点 refresh
 *     触发 AI 建议 + 人工 accept/reject」。agent 主动建议 affection/trust 等
 *     deltas 也是天然扩展。
 * 上面两条之所以暂未接：候选/状态 schema 比 entry 更复杂（candidate 要带优先级、
 * relationship suggestion 要带 5 个 delta + reason），加进来会让 schema 大不少。
 * 如果未来要做，按 docs/xingye-propose-draft.md 11 步清单照走即可——特别注意
 * 步骤 ④（heartbeat directive 菜单）和 ⑪（UI 集成测试），这两步没有不变量测试守。
 */
