...
function normalizeExports(body) {
  ...
  let inStr = null;        // 当前字符串/模板的引号字符（" ' `），否则 null
  let inLineComment = false;
  let inBlockComment = false;
  let inRegex = false;
  let depth = 0;           // {}/[]/() 嵌套深度；只在 depth-0 改写顶层语句的 export
  ...
  for (let i = 0; i < body.length; i++) {
    ...
    // 空白：换行视作 ASI 语句边界（顶层时），其余空白保持当前 statement-start 状态。
    if (/\s/.test(c)) {
      if (c === "\n" && depth === 0) atStatementStart = true;
      out += c;
      continue;
    }
    ...
    if (c === '"' || c === "'" || c === "`") { inStr = c; out += c; prevSignificant = c; atStatementStart = false; continue; }
    if (c === "/" && regexAllowedAfter(prevSignificant)) { inRegex = true; out += c; atStatementStart = false; continue; }
