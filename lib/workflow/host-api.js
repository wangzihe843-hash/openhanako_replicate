 ...
 *   onAgentEvent?: (evt: { phase: 'start'|'session'|'done'|'fail', nodeId: string, label?: string|null, agentId?: string|null, phaseLabel?: string|null, childSessionPath?: string|null }) => void,
...
export function createHostApi(deps) {
  ...
  async function agent(prompt, opts = {}) {
    ...
    return limiter.run(async () => {
      ...
      onAgentEvent({ phase: "start", nodeId, label, agentId: nodeAgentId, phaseLabel: currentPhase });
      // 子 session 创建后回补 childSessionPath（节点行展开看实时流用）。
      isoOpts.onSessionReady = (sp) => onAgentEvent({ phase: "session", nodeId, childSessionPath: sp });
      ...
      try {
        ...
        onAgentEvent({ phase: "done", nodeId });
        return result;
      } catch (err) {
        onAgentEvent({ phase: "fail", nodeId });
        throw err;
      }
    });
  }
