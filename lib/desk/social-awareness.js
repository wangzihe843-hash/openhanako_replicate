 ...
 *     或「某个已有明确关系的人太久没联系」(per-peer) 时，那一次心跳才追加一段软提示。
 *
...
/**
 * 距离上次 dm **某个已有明确关系的人** 累计 ≥ 这么多条用户对话，就把这个人单独列为
 * 「很久没联系」。默认 80：亲朋好友、恋人、宿敌、已发现的情敌等既定关系都不会被永久
 * 遗忘；没有关系 lore 的 peer 没有保底。可被 `desk.social_per_peer_threshold` 覆盖。
 */
...
/**
 * 读 peer-state.json。userTurnCount 是不会随 7 天 event retention 消失的累计用户对话序号；
 * lastOutboundDmTurn 是 global / per-peer 的序号基线。
 * 文件不存在 / 损坏 → 返回空壳，不抛。
 */
export function readPeerState(agentDir) {
  ...
  if (isRecord(raw.peers)) {
    for (const [peerId, entry] of Object.entries(raw.peers)) {
      ...
      const turn = normalizeTurnCount(entry.lastOutboundDmTurn);
      if (at || turn !== null) {
        peers[peerId] = {
          lastOutboundDmAt: at,
          lastOutboundDmTurn: turn,
        };
      }
    }
  }
  return {
    version: 2,
    userTurnCount: normalizeTurnCount(raw.userTurnCount) ?? 0,
    countedRecentChatEventIds: normalizeEventIds(raw.countedRecentChatEventIds),
    lastOutboundDmAt: normalizeDateString(raw.lastOutboundDmAt),
    lastOutboundDmTurn: normalizeTurnCount(raw.lastOutboundDmTurn),
    peers,
  };
}
...
export function recordOutboundDm({ agentDir, peerId, nowIso } = {}) {
  ...
  try {
    ...
    state.lastOutboundDmTurn = state.userTurnCount;
    state.peers = {
      ...state.peers,
      [peerId]: { lastOutboundDmAt: at, lastOutboundDmTurn: state.userTurnCount },
    };
...
/**
 * 把 event log 中尚未计入的 recent_chat.observed 合并进持久累计序号。
 * countedRecentChatEventIds 只保留当前 log 仍存在的 id：事件 7 天后被 prune 时，累计数仍在，
 * id 则安全丢弃；若 consumer 在标记 consumed 前重试，同一 id 也不会重复累计。
 */
export function syncPeerStateUserTurns({ agentDir, events } = {}) {
  if (!agentDir) return null;
  try {
    const state = readPeerState(agentDir);
    const chatEventsById = new Map();
    for (const event of Array.isArray(events) ? events : []) {
      if (event?.type !== "recent_chat.observed" || typeof event.id !== "string" || !event.id.trim()
        || !normalizeDateString(event.createdAt)) continue;
      const id = event.id.trim();
      if (!chatEventsById.has(id)) chatEventsById.set(id, event);
    }
    const chatEventIds = Array.from(chatEventsById.keys());
    const counted = new Set(state.countedRecentChatEventIds);
    let added = 0;
    for (const id of chatEventIds) {
      if (counted.has(id)) continue;
      counted.add(id);
      added += 1;
    }
    state.userTurnCount += added;
    // Older files only stored DM timestamps. Convert each missing baseline once
    // while the retained events are available, so recent DMs stay recent after
    // rereads and their elapsed turn counts survive later event pruning.
    const chatEvents = Array.from(chatEventsById.values());
    for (const entry of [state, ...Object.values(state.peers)]) {
      if (normalizeTurnCount(entry.lastOutboundDmTurn) !== null || !entry.lastOutboundDmAt) continue;
      const elapsedTurns = countChatTurnsSince(chatEvents, Date.parse(entry.lastOutboundDmAt));
      entry.lastOutboundDmTurn = Math.max(0, state.userTurnCount - elapsedTurns);
    }
    state.countedRecentChatEventIds = Array.from(new Set(chatEventIds));
    atomicWritePeerState(agentDir, state);
    return state;
  } catch {
    return null;
 ...
 * @param {Array<{id:string,name?:string,summary?:string,relationshipKnown?:boolean,relationshipLore?:string,socialFallbackMode?:string,socialFallbackTurnInterval?:number|null}>} opts.peers
 *   当前可联系的 peer 列表（**已排除自己**）。relationshipKnown=false 的 peer 不参与
 *   staleness 保底；undefined 只为旧调用方兼容，按可参与处理。
 ...
 *
 * 语义：
 *  - global：距离上次 dm 任何人累计 ≥ globalThreshold 条对话 → shouldSocialize
 *    （且至少有一个已有明确关系的 peer）
 *  - per-peer：已有明确关系的 peer 距离上次被 dm ≥ perPeerThreshold 条对话 → overdue
 *  - relationshipKnown=false：不进候选、不计 overdue、不会产生保底互动
 *  - candidatePeers：达到逐关系阈值时列出**全部 overdue**，让 3 个以上关系对象也都在
 *    第 80 条获得保底机会；只有 global 单独触发时才取 top-N 控制提示长度
...
export function computeSocialStaleness({
  ...
  const fallbackPeers = safePeers.filter((peer) => {
    const mode = normalizeString(peer.socialFallbackMode).toLowerCase();
    if (mode === 'disabled') return false;
    if (mode === 'enabled') return true;
    // Legacy callers omitted relationshipKnown; preserve their previous
    // eligible-by-default behavior. Production callers always pass true/false.
    return peer.relationshipKnown !== false;
  });
  const state = peerState && isRecord(peerState.peers)
    ? peerState
    : { lastOutboundDmAt: null, peers: {} };
  const persistentUserTurnCount = normalizeTurnCount(state.userTurnCount);

  const turnsSince = (entry, lastAt) => {
    const baseline = normalizeTurnCount(entry?.lastOutboundDmTurn);
    if (persistentUserTurnCount !== null && baseline !== null) {
      return Math.max(0, persistentUserTurnCount - baseline);
    }
    if (persistentUserTurnCount !== null && !lastAt) return persistentUserTurnCount;
    const sinceMs = lastAt ? Date.parse(lastAt) : -Infinity;
    return countChatTurnsSince(safeEvents, sinceMs);
  };

  // ── global ──
  const globalLastDmAt = normalizeDateString(state.lastOutboundDmAt);
  const globalChatTurns = turnsSince(state, globalLastDmAt);
  const shouldSocialize = fallbackPeers.length > 0 && globalChatTurns >= globalThreshold;

  // ── per-peer ──
  let overduePeerCount = 0;
  const ranked = fallbackPeers.map((peer) => {
    const entry = state.peers?.[peer.id];
    const lastDmAt = normalizeDateString(entry?.lastOutboundDmAt);
    const turns = turnsSince(entry, lastDmAt);
    const configuredInterval = peer.socialFallbackTurnInterval;
    const fallbackThreshold = configuredInterval === undefined || configuredInterval === null || configuredInterval === ''
      ? perPeerThreshold
      : clampThreshold(configuredInterval, perPeerThreshold);
    if (turns >= fallbackThreshold) overduePeerCount += 1;
    return {
      peerId: peer.id,
      ...
      relationshipKnown: peer.relationshipKnown === true,
      socialFallbackMode: normalizeString(peer.socialFallbackMode).toLowerCase() || 'auto',
      fallbackThreshold,
      ...(typeof peer.relationshipLore === 'string' && peer.relationshipLore.trim()
        ? { relationshipLore: peer.relationshipLore.trim() }
        : {}),
    };
  });
  ...
  return {
    ...
    shouldSocialize,
    overduePeerCount,
    relationshipPeerCount: fallbackPeers.filter((peer) => peer.relationshipKnown === true).length,
    fallbackPeerCount: fallbackPeers.length,
    // A peer-specific interval is authoritative. The lower global reminder may
    // say social contact is generally stale, but it must not nominate a peer
    // before that peer's own fallback interval has elapsed.
    candidatePeers: ranked.filter((peer) => peer.chatTurnsSinceLastDm >= peer.fallbackThreshold),
  };
}
