 ...
 * 导致「X 是谁」答不上来、甚至把 peer 当成用户。每个 agent 可定制 AGENTS.public.md
 * （对外意识，专为"给外人看"设计），这里把它紧凑化后塞进花名册，让 peer 之间能真正
 * 「对得上人设 / 对得上号」。
 *
 * 只读 peer 已落盘的 AGENTS.public.md；旧文件由启动迁移统一改名。
 * {{userName}}/{{agentName}}/{{agentId}} 占位符在本函数读取时填充；
 * 文件缺失/为空 → 返回 ""，调用方回退到 description.md。
 */

import path from "path";
import { safeReadFile } from "../../shared/safe-fs.ts";
import { PUBLIC_PERSONA_FILE_NAME } from "../../core/persona-source.ts";
...
export function readCompactPeerPersona({ agentsDir, peerId, peerName, userName, maxChars = PEER_PERSONA_MAX_CHARS } = {}) {
  ...
  const raw = safeReadFile(path.join(agentsDir, peerId, PUBLIC_PERSONA_FILE_NAME), "");
  ...
  const text = raw
    ...
    .replace(/^\s*[-+*]\s+/gm, "")        // markdown 列表项前缀（- / + / *），避免折行后留下 " - " 脏串
