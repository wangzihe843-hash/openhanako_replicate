 ...
 */
function buildHeartbeatContext({ deskChanged, changedFiles, overwatch, agentName, isZh, patrolLog, activityDir, patrolLogPath, xingyeEventSummary, autoDraftStaleness }) {
  const now = new Date();
  ...
  if (autoDraftStaleness?.mustPropose) {
    ...
    parts.push(isZh
      ? [
          ...
          "- `mail`（邮件草稿）/ `shopping`（购物候选）/ `secondhand`（二手出售候选）",
          ...
          "- `phone_contact`（通讯录更新候选：对**现有联系人**的 remark/impression/relationshipHint/tags/faction 做小步更新建议；**不新增联系人、不拉黑/删除**——那些走通讯录手动 AI 路径）",
          "- `sms`（短信草稿：拟一条草稿短信由用户确认后发出；可选 contact 引用，长度≤140 字）",
      ...
      : [
          ...
          "- `mail` (draft mail) / `shopping` (wishlist candidate) / `secondhand` (resale candidate)",
          ...
          "- `phone_contact` (contact-update candidate: small patches to an **existing contact**'s remark/impression/relationshipHint/tags/faction; **no adding contacts, no block/delete** — those go through the manual AI flow in the contacts panel)",
          "- `sms` (text-message draft for the user to confirm before sending; optional contact ref, ≤140 chars)",
