 ...
 */
function buildHeartbeatContext({ deskChanged, changedFiles, overwatch, agentName, isZh, patrolLog, xingyeEventSummary }) {
  const now = new Date();
  ...
  async function _doBeat() {
    try {
      ...
      let beatPayload = null;
      {
        ...
        const prompt = buildHeartbeatContext({ deskChanged, changedFiles, overwatch, agentName, isZh, patrolLog, xingyeEventSummary });
        ...
        devlog(`Phase 1: 工作空间巡检执行中...${deskChanged ? "" : " (无文件变化)"}`);
        {
          ...
          try {
            ...
            if (xingyeConsumed && !err.xingyeConsumed) err.xingyeConsumed = xingyeConsumed;
            throw err;
          } finally {
            clearTimeout(timer);
          }
      ...
      const salvagedPayload = err?.xingyeConsumed ? { xingyeConsumed: err.xingyeConsumed } : null;
