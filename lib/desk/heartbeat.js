 ...
 * @param {boolean} [opts.proposeDraftAvailable=true]
...
function buildHeartbeatContext({ deskChanged, changedFiles, overwatch, agentName, isZh, patrolLog, activityDir, patrolLogPath, xingyeEventSummary, autoDraftStaleness, socialStaleness, proposeDraftAvailable = true }) {
  ...
  if (xingyeEventSummary) {
     ...
     *
     * proposeDraftAvailable=false（用户关了 tool / 限定了 patrol_tools）时整段软引导都不出：
  ...
  if (socialStaleness && (socialStaleness.shouldSocialize || socialStaleness.overduePeerCount > 0)) {
    ...
    if (candidateLines.length > 0) {
      ...
      const loreBlocks = (socialStaleness.candidatePeers || [])
        .filter((p) => p && typeof p.relationshipLore === "string" && p.relationshipLore.trim())
        .map((p) => {
          const name = (p.name && p.name !== p.peerId) ? `${p.name}（${p.peerId}）` : p.peerId;
          return `${isZh ? "— 你与 " : "— Your relationship with "}${name}${isZh ? " 的关系设定：" : ":"}\n${p.relationshipLore.trim()}`;
        });
      if (loreBlocks.length > 0) {
        parts.push("");
        parts.push(isZh
          ? "（以下是你对其中几位的关系设定，按它来定开场的语气与分寸，不要无视也不要编造未写明的细节：）"
 ...
 * @param {() => (boolean|Promise<boolean>)} [opts.getProposeDraftAvailable]
 *   本轮巡检 session 里 `xingye_propose_draft` 这个 OPTIONAL tool 是否真的可用——每拍调用一次
...
export function createHeartbeat({
  ...
  async function _doBeat() {
    try {
       ...
       * 缺省 / 抛错一律按 true：默认 agent 没传这个回调，行为与改动前完全一致；判定失败时
       * 宁可保留 directive（最坏多花点 token）也不要因为一个布尔算挂了就静默吞掉主动产出能力。
       */
      let proposeDraftAvailable = true;
      if (typeof getProposeDraftAvailable === "function") {
        try {
          proposeDraftAvailable = (await getProposeDraftAvailable()) !== false;
        } catch (err) {
          devlog(`getProposeDraftAvailable 失败，按可用处理: ${err?.message || err}`, "error");
          proposeDraftAvailable = true;
        }
      }
