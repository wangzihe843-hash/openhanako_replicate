 ...
 */
function buildHeartbeatContext({ deskChanged, changedFiles, overwatch, agentName, isZh, patrolLog, xingyeEventSummary, autoDraftStaleness }) {
  const now = new Date();
  ...
  if (autoDraftStaleness?.mustPropose) {
    ...
    parts.push(isZh
      ...
      : [
          ...
          "- `mail` (draft mail) / `shopping` (wishlist candidate)",
...
export function createHeartbeat({
  ...
  async function _doBeat() {
    try {
      ...
      console.error(`[heartbeat] beat error: ${msg}`);
