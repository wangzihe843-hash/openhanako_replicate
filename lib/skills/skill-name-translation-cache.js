...
export async function translateSkillNamesWithCache({
  ...
  for (const name of requested) {
    ...
    /**
     * 显式 frontmatter 覆盖优先级最高：skill 作者在 SKILL.md 里写了
     * `display-name-{lang}: …` 就直接用，跳过 API 翻译 + 跳过缓存比对。
     * 这一条专治项目术语（如 `xingye` 被翻成「兴业」）的硬伤，作者比模型更知道
     * 自己的术语该怎么译。当前 SKILL.md 文件 hash 变了也不需要重算——override
     * 不依赖外部模型，是确定性的。
     */
    const override = usableText(skill?.displayNames?.[lang]);
    if (override) {
