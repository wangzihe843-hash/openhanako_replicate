...
function frontmatterMetadata(parsed) {
  // 注意 typeof null === "object"：空的 `metadata:`（或 `metadata: null`）会被 js-yaml 解析成
  // {metadata: null}，旧写法漏掉 null 会返回 null，下游 normalizeDefaultEnabled/normalizeDisplayNames
  // 解引用即抛 TypeError，被 catch 后把合法 SKILL.md 静默降级成 fallback 名 + 空描述（还误报解析失败）。
 ...
 *
 * 历史上 YAML 解析失败会被静默吞掉，导致 SKILL.md frontmatter 写错（漏引号、内层
...
export function parseSkillMetadata(content, fallbackName = "", onError) {
  ...
  try {
    ...
    if (typeof onError === "function") {
      try { onError(err); } catch { /* never let caller's logger break parse */ }
    }
    return meta;
  }
}
